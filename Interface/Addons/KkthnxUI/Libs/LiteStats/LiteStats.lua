...
ls:SetScript("OnEvent", function(_, event, addon)
	if event == "ADDON_LOADED" and addon == "KkthnxUI" then
...
if memory.enabled then
	...
	Inject("Memory", {
		...
		OnEnter = function(self)
			...
			if bandwidth ~= 0 then
				GameTooltip:AddDoubleLine(L_STATS_BANDWIDTH, format("%s ".."Mbps", K.Round(bandwidth, 2)), ttsubh.r, ttsubh.g, ttsubh.b, 1, 1, 1)
...
if gold.enabled then
	...
	Inject("Gold", {
		...
		OnEnter = function(self)
			...
			if C.stats.currency_raid and K.Level >= 100 then
				GameTooltip:AddLine(" ")
...
if stats.enabled then
	local function tags(sub)
		...
		if sub == "power" then
			...
			if attack > power and K.Class ~= "HUNTER" then
				value = attack
		...
		elseif sub == "manaregen" then
			local I5SR = true
			if K.Class == "ROGUE" or K.Class == "WARRIOR" or K.Class == "DEATHKNIGHT" then
				string, percent = "??"
			else
