...
local function LimitLines()
	for i = 1, #ct.frames do
		f = ct.frames[i]
		f:SetMaxLines(f:GetHeight() / C.font.combat_font_size)
	end
end
...
if C.combattext.damage_style then
	DAMAGE_TEXT_FONT = C.font.combat_font
end
...
for i = 1, numf do
	...
	f:SetFont(C.font.combat_font, C.font.combat_font_size, C.font.combat_font_style)
	f:SetShadowColor(0, 0, 0, C.font.combat_font_shadow and 1 or 0)
	f:SetShadowOffset(C.font.combat_font_shadow and 1 or 0, C.font.combat_font_shadow and -1 or 0)
	...
	f:SetClampRectInsets(0, 0, C.font.combat_font_size, 0)
...
if C.combattext.dk_runes and K.Class == "DEATHKNIGHT" then
...
local StartConfigmode = function()
	if not InCombatLockdown()then
		for i = 1, #ct.frames do
			...
			f.fs:SetFont(C.font.combat_font, C.font.combat_font_size, C.font.combat_font_style)
			...
			if not C.combattext.scrollable then
				f:SetScript("OnSizeChanged", function(self)
					self:SetMaxLines(self:GetHeight() / C.font.combat_font_size)
					self:Clear()
				end)
			end
		...
		if (sourceGUID == ct.pguid and destGUID ~= ct.pguid) or (sourceGUID == UnitGUID("pet") and C.combattext.pet_damage) or (sourceFlags == gflags) then
				...
				if amount >= C.combattext.treshold then
					...
					if C.combattext.merge_aoe_spam and C.combattext.merge_melee then
						...
						if SQ[spellId]["count"] == 1 then
							SQ[spellId]["utime"] = time() + K.aoespam[spellId]
						end
			...
			elseif eventType == "SPELL_DAMAGE" or (eventType == "SPELL_PERIODIC_DAMAGE" and C.combattext.dot_damage) then
				...
				if amount >= C.combattext.treshold then
					...
					if C.combattext.merge_aoe_spam then
						spellId = K.merge[spellId] or spellId
						if K.aoespam[spellId] then
							SQ[spellId]["locked"] = true
							SQ[spellId]["queue"] = ct.SpamQueue(spellId, rawamount)
							SQ[spellId]["msg"] = msg
							SQ[spellId]["color"] = color
