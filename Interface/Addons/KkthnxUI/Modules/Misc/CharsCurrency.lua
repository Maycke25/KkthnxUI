...
if C.misc.charscurrency ~= true then return end
...
frame:SetScript("OnEvent", function(self, event, addon)
	if event == "ADDON_LOADED" then
		...
		if not SavedCurrency[K.Realm] then SavedCurrency[K.Realm] = {} end
		if not SavedCurrency[K.Realm][faction] then SavedCurrency[K.Realm][faction] = {} end
		if not SavedCurrency[K.Realm][faction][K.Realm] then SavedCurrency[K.Realm][faction][K.Realm] = {} end

		for k, v in pairs(SavedCurrency[K.Realm]) do
			if k ~= "Alliance" and k ~= "Horde" then
				SavedCurrency[K.Realm][k] = nil
			end
		end

		local now = time()

		realmDB = SavedCurrency[K.Realm][faction]
		if not realmDB then return end
		charDB = realmDB[K.Realm]
		...
		for name, data in pairs(realmDB) do
			if data.lastSeen and data.lastSeen < cutoff then
				realmDB[name] = nil
			elseif name ~= K.Name then
				tinsert(playerList, name)
			end
		end
		sort(playerList)
		tinsert(playerList, 1, K.Name)
