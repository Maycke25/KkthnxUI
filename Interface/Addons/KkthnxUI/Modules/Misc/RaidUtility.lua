...
if C.misc.raidtools ~= true then return end
...
RaidUtilityPanel:CreatePanel("Transparent", 170, 145, unpack(C.position.raidutility))
...
local function CreateButton(name, parent, template, width, height, point, relativeto, point2, xOfs, yOfs, text)
	...
	if text then
		...
		b.t:SetFont(C.font.basic_font, C.font.basic_font_size, C.font.basic_font_style)
		b.t:SetPoint("CENTER")
