...
local createIconButton = function (name, parent, texture, point, hint, isBag)
	...
	button.tooltip:SetFont(font, 12, "OUTLINE")
...
function MyContainer:OnCreate(name, settings)
	...
	background:SetBackdropColor(1, 1, 1, .9)
	background:SetBackdropBorderColor(.7, .7, .7, 1)
	
	background:SetPoint("TOPLEFT", -5, 5)
	background:SetPoint("BOTTOMRIGHT", 5, -5)
	
	-- Stripes
	background.tex = "Interface\\AddOns\\cargBags_Nivaya\\media\\StripesThin"
	
	-- Caption, close button
	local caption = background:CreateFontString(background, "OVERLAY", nil)
	caption:SetFont(font, 12, "OUTLINE")
	if(caption) then
	...
	if (tBag or tBank or tReagent) then
		...
		self.DropTarget.bg:SetBackdropBorderColor(1, 1, 1, 1)
		...
		fs:SetJustifyH("LEFT")
		fs:SetPoint("CENTER", self.DropTarget, "CENTER", 1.5, 1.5)
	...
	if tBag then
		...
		self.hint:SetFont(font, 11, "OUTLINE")
