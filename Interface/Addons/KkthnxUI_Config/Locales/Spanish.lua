if GetLocale() ~= "esES" and GetLocale() ~= "esMX" then return end

----------------------------------------------------------------------------------------
--	Localization for esES and esMX client
----------------------------------------------------------------------------------------
L_GUI_BUTTON_RESET = "Total Reset UI"
L_GUI_SET_SAVED_SETTTINGS = "Set settings Per-Character"
L_GUI_RESET_CHAR = "Are you sure you want to reset your character's settings for |cFF4488FFKkthnx's|r |cFFFEB200UI|r?"
L_GUI_RESET_ALL = "Are you sure you want to reset all settings for |cFF4488FFKkthnx's|r |cFFFEB200UI|r"
L_GUI_PER_CHAR = "Are you sure you want to change to or from 'Per Char Settings'?"
L_GUI_MAKE_SELECTION = "You must make a selection before you can continue to config."

-- General options
L_GUI_GENERAL_AUTOSCALE = "Auto UI Scale"
L_GUI_GENERAL_UISCALE = "UI Scale (if auto-scale is disabled)"
L_GUI_GENERAL_WELCOME_MESSAGE = "Welcome message in chat > Welcome to |cFF4488FFKkthnx's|r |cFFFEB200UI|r"
L_GUI_GENERAL_LAG_TOLERANCE = "Auto update Blizzard Custom Lag Tolerance to your latency"

-- Miscellaneous options
L_GUI_MISC_SPIN_CAMERA = "Spin camera when you go afk"
L_GUI_MISC_PROFESSION_TABS = "Professions tabs on tradeskill & trade frames"
L_GUI_MISC_HIDE_BG_SPAM = "Remove Boss Emote spam during BGs"
L_GUI_MISC_PAPERDOLLSTATS = "Item level & durability on character frame"
L_GUI_MISC_ALREADY_KNOWN = "Colorizes recipes, mounts & pets that are already known"
L_GUI_MISC_DISENCHANTING = "Milling, Prospecting and Disenchanting by Alt + click"
L_GUI_MISC_ENCHANTMENT_SCROLL = "Enchantment scroll on TradeSkill frame"
L_GUI_MISC_STYLE_TRACKER = "Style the quest objective tracker. Makes it look fancy"
L_GUI_MISC_MOVE_TRACKER = "Move the quest objective tracker with shift + click to drag"
L_GUI_MISC_HATTRICK = "Adds checkboxes to toggle the helm and cloak settings"
L_GUI_MISC_SHORTGOLD = "Shorten the display of gold > 5g, 10s, 1c"
L_GUI_AUTOMATION_COLLECTGARBAGE = "Auto collect garbage to keep your UI running fast"
L_GUI_MISC_LFGQUEUETIMER = "Simple timer for your LFG pop as to when it will expire"
L_GUI_MISC_BOSSBANNER = "Prevents the big Boss Defeated Banner from appearing"

-- Announcements options
L_GUI_ANNOUNCEMENTS = "Announcements"
L_GUI_ANNOUNCEMENTS_INTERRUPTS = "Announce in party & raid when you interrupt"
L_GUI_ANNOUNCEMENTS_SAYSAPPED = "Announce in say when you are sapped"
L_GUI_ANNOUNCEMENTS_BAD_GEAR = "Check bad gear in instances"

-- Blizzard options
L_GUI_BLIZZARD = "Blizzard"
L_GUI_BLIZZARD_CAPTUREBAR = "Style & move Worldstate Capturebar"
L_GUI_BLIZZARD_ALTPOWERBAR = "Style & move Altpowerbar Capturebar"
L_GUI_BLIZZARD_ACHIEVEMENTS = "Style & move Achievement frame"
L_GUI_BLIZZARD_MOVE_BLIZZARD = "Move most Blizzard frames"

L_GUI_MINIMAP = "Minimap"
L_GUI_MINIMAP_ENABLEMINIMAP = "Enable minimap, and make it square"
L_GUI_MINIMAP_MINIMAPSIZE = "Minimap size - Default is 130"
L_GUI_MINIMAP_INFOLINE = "Infoline to show Location, Latency & FPS"
L_GUI_MINIMAP_COLLECTBUTTONS = "Collect the minimap buttons and syle them"

	-- Buffs options
L_GUI_BUFFS = "Buffs & Debuffs"
L_GUI_BUFFS_BUFFBORDERCOLOR = "Color of the border of the buffs (excluding debuffs)"
L_GUI_BUFFS_BUFFCOUNTSIZE = "Buff count size"
L_GUI_BUFFS_BUFFFONTSIZE = "Buff font size"
L_GUI_BUFFS_BUFFPERROW = "How many buffs per 1 row"
L_GUI_BUFFS_BUFFSCALE = "Scale of buffs"
L_GUI_BUFFS_BUFFSIZE = "Size of buffs"
L_GUI_BUFFS_DEBUFFCOUNTSIZE = "Debuff count size"
L_GUI_BUFFS_DEBUFFFONTSIZE = "Debuff font size"
L_GUI_BUFFS_DEBUFFSCALE = "Scale of debuffs"
L_GUI_BUFFS_DEBUFFSIZE = "Size of debuffs"
L_GUI_BUFFS_ENABLE = "Enable buffs and debuffs"
L_GUI_BUFFS_PADDINGX = "Buffs paddingx (Left and Right)"
L_GUI_BUFFS_PADDINGY = "Buffs paddingx (Up and Down)"

-- Tooltip options
L_GUI_TOOLTIP = "Tooltip"
L_GUI_TOOLTIP_ENABLE = "Enable the tooltip"
L_GUI_TOOLTIP_SPELLID = "Show spell IDs of items and spells"
L_GUI_TOOLTIP_FADE = "Show the tooltip fade"
L_GUI_TOOLTIP_HIDEINCOMBAT = "Hide the tooltip in combat (Can cause errors)"
L_GUI_TOOLTIP_USERPLACED = "Place the tooltip where you personally like."
L_GUI_TOOLTIP_REACTIONBORDERCOLOR = "Color the tip border of reaction of your foes"
L_GUI_TOOLTIP_ITEMQUALITYBORDERCOLOR = "Color the tip border of item quality (Purple, Blue, Green +)"
L_GUI_TOOLTIP_ABBREVREALMNAMES = "Shorten realm names on the tooltip to (*)"
L_GUI_TOOLTIP_HIDEREALMTEXT = "Completely hide the realm names on tooltips"
L_GUI_TOOLTIP_SHOWPLAYERTITLES = "Show player titles"
L_GUI_TOOLTIP_SHOWUNITROLE = "Show a players role"
L_GUI_TOOLTIP_SHOWPVPICONS = "Show pvp icons"
L_GUI_TOOLTIP_MOUSEOVERTARGET = "Show who is mousing over who."
L_GUI_TOOLTIP_SHOWSPECICON = "Show spec icon"
L_GUI_TOOLTIP_ICONS = "Show icons next to the title in the tooltip"
L_GUI_TOOLTIP_SHOWHEALTHVALUE = "Show helth value on tip healthbar"

-- Automation options
L_GUI_AUTOMATION = "Automation"
L_GUI_AUTOMATION_RESURRECTION = "Auto resurrection in battlegrounds"
L_GUI_AUTOMATION_SCREENSHOT = "Take screenshot when player gets achievement"
L_GUI_AUTOMATION_ACCEPTINVITE = "Auto accept invites (Friends / Guild)"
L_GUI_AUTOMATION_DECLINEDUEL = "Auto decline all duels"
L_GUI_AUTOMATION_AUTOCOLLAPSE = "Auto collapse objective tracker in instances"
L_GUI_AUTOMATION_BANNER_HIDE = "Auto hide Boss Banner Loot Frame"

-- Chat options
L_GUI_CHAT_ENABLE = "Enable chat"
L_GUI_CHAT_SPAM = "Removing some systems spam('Player1' won duel 'Player2')"
L_GUI_CHAT_GOLD = "Removing some players spam"
L_GUI_CHAT_WIDTH = "Chat width"
L_GUI_CHAT_HEIGHT = "Chat height"
L_GUI_CHAT_TIMESTAMP = "Timestamp coloring"
L_GUI_CHAT_WHISP = "Sound when whisper"
L_GUI_CHAT_SKIN_BUBBLE = "Skin Blizzard chat bubbles"
L_GUI_CHAT_CL_TAB = "Show Combat Log tab"
L_GUI_CHAT_TABS_MOUSEOVER = "Chat tabs on mouseover"
L_GUI_CHAT_STICKY = "Remember last channel"
L_GUI_CHAT_CHANNELREPLACE = "Replace channel names and shorten them [General] > [GEN]"

-- Nameplate options
L_GUI_NAMEPLATE_ENABLE = "Enable nameplates"
L_GUI_NAMEPLATE_COMBAT = "Automatically show nameplates in combat"
L_GUI_NAMEPLATE_HEALTH = "Numeral health value"
L_GUI_NAMEPLATE_CASTBAR = "Show nameplate castbar"
L_GUI_NAMEPLATE_HEIGHT = "Nameplates height"
L_GUI_NAMEPLATE_WIDTH = "Nameplates width"
L_GUI_NAMEPLATE_AD_HEIGHT = "Additional height for selected nameplate"
L_GUI_NAMEPLATE_AD_WIDTH = "Additional width for selected nameplate"
L_GUI_NAMEPLATE_CASTBAR_NAME = "Show castbar name"
L_GUI_NAMEPLATE_THREAT = "Enable threat feature, automatically changes by your role"
L_GUI_NAMEPLATE_CLASS_ICON = "Icons by class in PvP"
L_GUI_NAMEPLATE_NAME_ABBREV = "Display abbreviated names (show debuffs must be turned off)"
L_GUI_NAMEPLATE_GOOD_COLOR = "Good threat color, varies depending if your a tank or dps/heal"
L_GUI_NAMEPLATE_NEAR_COLOR = "Losing/Gaining threat color"
L_GUI_NAMEPLATE_BAD_COLOR = "Bad threat color, varies depending if your a tank or dps/heal"
L_GUI_NAMEPLATE_SHOW_DEBUFFS = "Show debuffs (from the list)"
L_GUI_NAMEPLATE_DEBUFFS_SIZE = "Debuffs size"
L_GUI_NAMEPLATE_HEALER_ICON = "Show icon above enemy healers nameplate in battlegrounds"

-- Error options
L_GUI_ERROR = "Errors"
L_GUI_ERROR_BLACK = "Hide errors from black list"
L_GUI_ERROR_WHITE = "Show errors from white list"
L_GUI_ERROR_HIDE_COMBAT = "Hide all errors in combat"

-- Loot options
L_GUI_LOOT_ENABLE = "Enable loot frame"
L_GUI_LOOT_ROLL_ENABLE = "Enable group loot frame"
L_GUI_LOOT_ICON_SIZE = "Icon size"
L_GUI_LOOT_WIDTH = "Loot frame width"
L_GUI_LOOT_AUTOGREED = "Enable auto-greed/disenchant for green item at max level"
L_GUI_LOOT_AUTODE = "Auto confirm disenchant"