...
local function CreateRollFrame()
	...
	status:SetStatusBarTexture("Interface\\Addons\\KkthnxUI_Media\\Media\\Textures\\KkthnxTex.tga")
	...
	bind:SetFont("Fonts\\FRIZQT__.TTF", 13, "OUTLINE")
	frame.fsbind = bind
...
CreateBorder(anchor, 12, 1)
anchor:SetBackdropColor(0, 0, 0, 0.8);
anchor:SetBorderColor(1.0, 0.82, 0)
local label = anchor:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall")
label:SetAllPoints(anchor)
label:SetText("LootGroup")
...
local function START_LOOT_ROLL(rollid, time)
	...
	CreateBorder(f.buttonborder, 12, -1)
	f.buttonborder:SetBorderColor(color.r, color.g, color.b)
...
anchor:SetScript("OnEvent", function(frame, event, addon)
	if addon ~= "KkthnxUI_Loot" then return end
	...
	if not LootDB then LootDB = {} end
	anchor.db = LootDB
	anchor:SetPoint("CENTER", UIParent, anchor.db.x and "BOTTOMLEFT" or "BOTTOM", anchor.db.x or 0, anchor.db.y or 221)
end)

SlashCmdList["LOOTGROUP"] = function() if anchor:IsVisible() then anchor:Hide() else anchor:Show() end end
SLASH_LOOTGROUP1 = "/lootgroup"
SLASH_LOOTGROUP2 = "/lg"
SLASH_LOOTGROUP3 = "/kloot"