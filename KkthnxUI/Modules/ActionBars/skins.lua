...
for i = 1, NUM_OVERRIDE_BUTTONS do
    ...
    if (cfg.button.showVehicleKeybinds) then
        hotkey:SetFont(C.font.action_bars_font, C.font.action_bars_fontsize + 3, 'OUTLINE')
        ...
        if (not button.Background) then
            ...
            if (macroname) then
                if (not C.actionbar.showmacroname) then
                    ...
                    macroname:SetFont(C.font.bar_macro_font, C.font.bar_macro_font_size, C.font.bar_macro_font_style)
                    macroname:SetShadowOffset(0,0)
